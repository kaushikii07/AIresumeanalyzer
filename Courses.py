...
course_map = {
    "machine learning": ds_course[:3],
    "data science": ds_course[3:6],
    "python": ds_course[5:7],
    "web development": web_course[:3],
    "react": web_course[2:5],
    "javascript": web_course[2:3],
    "android": android_course[:3],
    "kotlin": [android_course[3]],
    "flutter": android_course[8:10],
    "ios": ios_course[:3],
    "swift": ios_course[7:9],
    "ui/ux": uiux_course[:3],
    "design": uiux_course[:3],
}
...
def recommend_courses(skills):
    ...
    for skill in skills:
        if skill in course_map:
            recommended.extend(course_map[skill])

    return list({course[1]: course for course in recommended}.values())  # Remove duplicates by URL
